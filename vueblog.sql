...
CREATE TABLE `vue_blog` (
  ...
  `articleSubTitle` mediumtext NOT NULL COMMENT '文章的二级标题，用于搜索快速定位',
  `articleNature` int(11) NOT NULL DEFAULT '0' COMMENT '0: 原创 1：转载 2：翻译',
  `articleKey` varchar(255) NOT NULL COMMENT '文章关键字',
  `articleContentMarkdown` longtext NOT NULL COMMENT '文章内容markdown格式',
  `articleContentHtml` longtext NOT NULL COMMENT '文章内容html格式',
...
-- ----------------------------
-- Table structure for vue_blog_ad
-- ----------------------------
DROP TABLE IF EXISTS `vue_blog_ad`;
CREATE TABLE `vue_blog_ad` (
  `adId` int(10) unsigned NOT NULL AUTO_INCREMENT COMMENT '广告位 id',
  `adName` varchar(255) NOT NULL COMMENT '广告位的名称',
  `adUrl` varchar(255) NOT NULL COMMENT '广告位链接',
  `adPcImg` varchar(255) NOT NULL COMMENT 'PC端的广告图链接',
  `adMImg` varchar(255) CHARACTER SET utf8 COLLATE utf8_general_ci DEFAULT NULL COMMENT 'M端的广告图链接',
  `adSort` int(11) NOT NULL COMMENT '广告位排序',
  PRIMARY KEY (`adId`)
...
CREATE TABLE `vue_blog_author` (
  ...
  `authorHeadimg` varchar(255) DEFAULT NULL COMMENT '作者头像链接',
  `authorIntroduce` varchar(255) DEFAULT NULL COMMENT '作者简介',
  `authorPassword` varchar(255) NOT NULL COMMENT '密码',
  `authorEmail` varchar(255) DEFAULT NULL COMMENT '作者邮箱',
...
CREATE TABLE `vue_blog_column` (
  ...
  `columnSort` int(11) NOT NULL DEFAULT '1000' COMMENT '栏目的排序',
...
CREATE TABLE `vue_blog_img` (
  ...
  `articleId` int(11) DEFAULT NULL COMMENT '图片所属的文章id/广告位id',
...
CREATE TABLE `vue_blog_title` (
  ...
  `h0` longtext COMMENT '文章标题',
  `h1` longtext COMMENT '一级标题',
  `h2` longtext COMMENT '二级标题',
  `h3` longtext COMMENT '三级标题',
  `h4` longtext COMMENT '四级标题',
  `h5` longtext COMMENT '五级标题',
  `h6` longtext COMMENT '六级标题',
  PRIMARY KEY (`id`)
...
CREATE TABLE `vue_blog_views` (
  ...
  `routeFrom` varchar(255) NOT NULL COMMENT '对应router.beforeEach中from.path',
  `routeTo` varchar(255) NOT NULL COMMENT '对应router.beforeEach中to.path',
  `time` datetime DEFAULT NULL ON UPDATE CURRENT_TIMESTAMP COMMENT '跳转时间',
  `clientSystem` varchar(255) DEFAULT NULL COMMENT '客户端系统',
  `clientBrowser` varchar(255) DEFAULT NULL COMMENT '客户端浏览器',
  `clientBrowserVersion` varchar(255) DEFAULT NULL COMMENT '客户端浏览器版本号',
